...
```json
{
  "rust-analyzer.cargo.features": ["system-nucleo-f401re", "sw-report"],
...
Nucleo uses TIM2 for software dispatch and TIM3 for the report-trigger
interrupt. Its LED blink uses SysTick. Default systems and chip-only builds use
TIM2 and TIM3 as dispatchers. See [hardware and
systems](../architecture/hardware-and-systems.md).
...
No hardware was flashed during this investigation. The Nucleo's physical
initialization now lives in its system module and board manifest. The editor
selection additionally enables the independent `sw-report` software task.
The system separation adds default systems, with the current matrix recorded
in [test-report.md](../verification/test-report.md). Source line numbers above refer to the
...
- The reducer supports registered chip/system/software feature predicates and
  the authorized prototype's board predicates. It reduces module items, struct
  fields, struct-literal fields, and `let` statements.
  ...
  new systems requires registering them; new axes require explicit DSL
  permission plus evaluator tests. This is not a general Rust
