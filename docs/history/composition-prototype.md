...
> Historical composition-fixture notes. The normal app now uses a
> source-preserving attribute macro; see [IDE
> workflow](../development/ide-workflow.md) and the root
> [README](../../README.md). The text composer remains for regression
> fixtures.
...
`docs/verification/test-report.md`. `Cargo.lock` is retained as part of the test artifact after the first
run.
