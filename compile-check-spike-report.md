...
> using a span-preserving attribute macro. See [IDE workflow and verification](docs/ide-workflow.md).
...
1. **Type-alias skeleton + chip-level file selection** (now archived at
   `archive/option-a-type-alias/` — see §6) — no extra tooling, but forces a union
